...
#############################################################################################
#Script Name   :normal_base_git_src_pull.sh                                                              
#Author        :DIVAGAR N                                                
#Email         :n.divagar@mobiveil.co.in                                          
#############################################################################################

# Pull normal base Docker image
sudo docker pull nimbushaystack/haystack_noetic_image_base:v0.0

sudo docker stop haystack
sudo docker rm haystack

# Run a new container from the image and name it
sudo docker run -it -w /haystack_ws/ -d --env /opt/ros/noetic/setup.bash --privileged --name "haystack" nimbushaystack/haystack_noetic_image_base:v0.0
